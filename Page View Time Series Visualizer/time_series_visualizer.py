...
def draw_line_plot():
  ...
  line_plot = sns.lineplot(data=df, x='date', y='value')
...
def draw_bar_plot():
  ...
  df_bar = df.copy()
  df_bar['year'] = pd.to_datetime(df_bar.index).year
  df_bar['month'] = pd.to_datetime(df_bar.index).strftime('%B')
  ...
  # Draw bar plot
  fig = df_bar.plot(kind='bar', figsize=(10, 10))
